...
- The gate requires a verified `TransactionVerificationRecord` (`src/lib/duelogic/transaction-verification.ts`, read-only repository contract) bound to the exact intervention, merchant, payer, subscription, selected date, both stored schedules and policyVersion, unexpired and unconsumed. NO write path for these records exists anywhere: the development repository always returns null, so `finalConfirmationEnabled` (derived from the verified record, never hardcoded) is false, the rendered confirm button stays disabled with verification-required wording, the dev route `POST /api/duelogic/dev/interventions/confirm` refuses with stage `verification-required`, and token possession alone can never initiate execution. The later OTP stage creates and verifies these records without changing `confirmInterventionExecution`.
...
- The intervention flow lives in `src/lib/duelogic/intervention-service.ts` with injected repositories, clock, token functions, read-only Pinch effects (`intervention-pinch-reads.ts`, GET only) and the injected replacement-path invoker; the store (`dev-intervention-store.ts`) is process-local, non-durable sandbox memory; the fifteen-scenario deterministic suite (`intervention-validation.ts`) never calls Pinch — the internal execution scenarios drive the real replacement flow with injected fakes only, and the gate-refusal scenario proves missing verification blocks execution — and is re-asserted by the scan route and the dashboard render.
...
- The frozen default remains the builder's default argument: scheduled interventions and customer date evaluation still use the no-argument frozen-default path (`intervention-service.ts`); intervention policy binding is the next controlled policy stage. Transaction verification and gated execution are unaffected by policy adoption — activation creates no verification record and cannot open the execution gate. The process-local development-store limitation remains. The twenty-one-scenario suite (`policy-snapshot-validation.ts`) is re-asserted by the dashboard render and the dev policy route.
