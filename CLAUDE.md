...
- No consumer has been switched from the frozen policy yet: historical-replay adoption, opportunity integration and intervention policy binding remain future controlled stages, so dashboard replay and opportunity figures do not change when a new version is activated. Process-local persistence remains a development limitation. The thirteen-scenario suite (`policy-snapshot-validation.ts`) is re-asserted by the dashboard render and the dev policy route.
