...
- The frozen default remains the builder's default argument: scheduled interventions and customer date evaluation still use the no-argument frozen-default path (`intervention-service.ts`) until the next controlled policy-binding stage. The process-local development-store limitation remains. The twenty-scenario suite (`policy-snapshot-validation.ts`) is re-asserted by the dashboard render and the dev policy route.
